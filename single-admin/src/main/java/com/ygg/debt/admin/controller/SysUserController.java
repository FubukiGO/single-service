...
import com.ygg.debt.admin.model.dto.UserDTO;
import com.ygg.debt.admin.model.entity.SysUser;
import com.ygg.debt.admin.model.entity.SysUserRole;
...
public class SysUserController extends BaseController {
    ...
    public R<Page> page(@RequestBody Map<String, Object> params) {
        ...
        Page<User> page = sysUserService.selectUserPage(new Query<>(params), params);
    ...
    public R<Boolean> user(@RequestBody UserDTO userDto) {
        ...
        if (check.selectCount(new EntityWrapper<>(check)) > 0) throw new BusinessException("用户名已存在");
    ...
    public R<Boolean> edit(@RequestBody UserDTO userDto) {
        ...
        check.setUsername(userDto.getUsername());
        if (check.getUsername() == null && check.selectCount(new EntityWrapper<>(check)) > 0)
...
//            return new R<>(false, "手机号已被注册");
        ...
        if (CollectionUtil.isNotEmpty(userDto.getRole())) {
            ...
            userRole.delete(new EntityWrapper<>(userRole));
